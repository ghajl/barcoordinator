...
export async function getUserPlacesData(req, res) {
  ...
  try {
    ...
    const visitors = found.map(bar => ({
      barId: bar.barId,
      count: bar.users.length
    }));
    const {
      username = null,
      profile = null,
      id = null,
      bars: currentUserBars = null
    } = req.user || {};
    return res.json({
      visitors,
      currentUserBars,
...
export async function getUserData(req, res) {
  ...
  const { username = null, profile = null, id = null, bars = null } = req.user;
  try {
    return res.json({
      username,
      profile,
  ...
  try {
    ...
    const result = users.map(userdata => {
      if (
        ...
        (userdata.profile.givenName != null ||
          userdata.profile.familyName != null)
      ) {
...
export async function getUserBasket(req, res) {
  ...
  const { username, profile, id, bars } = req.user;
  try {
...
export async function authFacebookReturn(req, res) {
  const { id } = req.user;
  let addOperation = false;
  let addOperationSuccess = false;
  if (req.session.bar) {
    addOperation = true;
    const userId = id;
    const { barId, placeId } = req.session.bar;
    req.session.bar = null;
    try {
      await Bar.findOneAndUpdate(
        { barId },
        { $addToSet: { users: userId }, placeId },
  ...
  let query = `?addOperation=${addOperation}`;
