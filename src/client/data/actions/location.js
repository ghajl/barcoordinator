  ...
  return dispatch => {
    ...
    return geocodeByAddress(address)
      ...
      .catch(() => {
        dispatch(
          locationSearchError(
            'Your request could not be completed, something wrong with the address'
          )
        );
      });
  };
...
export function showBars(address) {
      ...
      .then(({ lat, lng }) => {
        ...
        mapService.nearbySearch(request, (results, status) => {
            ...
            return axiosInstance
              ...
              .then(response => {
                ...
                response.data.visitors.forEach(bar => {
                  const i = locationBars.findIndex(
                    elem => elem.id === bar.barId
                  );
          ...
          if (status === google.maps.places.PlacesServiceStatus.ZERO_RESULTS) {
            return dispatch(
              zeroResultsSearchError('Sorry, no matches were found')
            );
          }
          dispatch(barsSearchError('Unable to show results'));
        });
