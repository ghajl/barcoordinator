...
export function fetchUserData() {
  return dispatch => {
    ...
    return axiosInstance
      .get('/currentUser')
      .then(response => {
        const { username, profile, userId, bars } = response.data;
...
export function showBasket() {
  return (dispatch, getState) => {
    ...
    return axiosInstance
      .get('/currentUserBasket')
      .then(async response => {
        const { currentUserBars: bars } = response.data;
        const mapService = getState().reducer.map.service;
        const basketList = [];
        await bars.reduce(
          (p, bar) =>
            p.then(async () => {
              const barDetails = await getBarDetails(mapService, bar.placeId);
              const item = {
                barId: bar.barId,
                placeId: bar.placeId,
...
export function facebookLoginAddSuccess() {
  return {
    type: actionTypes.FACEBOOK_LOGIN_ADD_SUCCESS,
    message: 'You have successfully added to the list!'
  };
}

export function facebookLoginAddError() {
  return {
    type: actionTypes.FACEBOOK_LOGIN_ADD_ERROR,
